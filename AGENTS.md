...
- The fork is branded **Customcode**: `CFBundleDisplayName`/`CFBundleName` in `Info.plist`, `WindowTitle.appName`, the main `Window` scene title, and the app-menu "Customcode" / "Quit Customcode" entries in `supacodeApp.swift`, plus the CC artwork in `AppIcon.appiconset`. `PRODUCT_NAME` (`supacode.app`), the bundle ID, deeplink scheme, and CLI name are deliberately unchanged so Sparkle updates, saved settings, and scripts keep working. Alert/settings prose still says "Supacode" on purpose — renaming it would bloat the upstream-merge surface. Keep all of this when merging upstream.
