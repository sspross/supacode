...
## Repository groups (fork feature)

- User-defined, collapsible sidebar groups of repositories. Persistence lives in `SidebarState`: `groups: OrderedDictionary<SidebarGroupID, Group>` (name + collapsed) plus an optional `groupID` on each repo `Section`. There is exactly ONE ordering source of truth — `sections` key order. Groups have no own position: a group renders at the position of its first member, and the mutation API (`assign(repository:toGroup:)`, `createGroup`, `reconcileGroupMembership(afterMoving:ordered:)`) keeps a group's members contiguous in `sections` order by construction. `buildRepositorySections` pulls stray non-contiguous members together at render time as a safety net against hand-edited state.
- Groups are non-empty by construction: `createGroup` no-ops on an empty member list, and any path that removes a group's last member (`assign` to nil, drag-away reconciliation) auto-deletes the group. `dissolveGroup` ("Ungroup") clears membership in place. A `Section.groupID` pointing at a missing `groups` entry reads as "not grouped" via `SidebarState.groupID(of:)` — never read `section.groupID` directly.
- The header renders as a content-less `Section { } header: { SidebarRepoGroupHeaderRow }`, NEVER as a bare list row: a bare row gets an implicit list section whose extra boundary makes header → first-member ~14pt looser than repo → repo (prototype-measured), and no macOS API can shrink it (`listSectionSpacing` / `listRowSpacing` / `listSectionMargins` are all `macOS, unavailable`, macOS 26.2 SDK). Section headers ignore `listRowInsets`; spacing is tuned via `SidebarNestLayout.groupHeaderTopPadding` / `groupHeaderBottomPadding` padding on the header content. The header stays one ForEach element at the same flat index, so the header-drag whole-group move is unaffected.
- `SidebarStructure.Section.repoGroupHeader` carries name, collapse state, `memberRepositoryIDs` (always) and `leafRowIDs` (only when collapsed). A collapsed group's member repo sections are omitted from `sections` entirely, which drops their rows out of hotkey numbering / arrow nav for free; the header renders aggregated activity indicators over `leafRowIDs`, reusing `SidebarPathGroupAggregatedIndicators` (internal for this reason) so per-leaf observation stays bounded. `reorderableRepositoryIDs` still mirrors `orderedRepositoryIDs()` 1:1.
- Drag semantics, all decided in `SidebarState.reconcileGroupMembership` after `.repositoriesMoved`: a repo dropped strictly inside a group's run (both neighbors in the group) joins it; a member moved to its own run's edge (one neighbor still a sibling) stays; a member dragged away leaves. Dragging the group header row moves the whole run via `.sidebarGroupMoved` (the reducer resolves member offsets itself, skipping membership reconciliation so a single-member group survives its own move).
- Reducer arms live in `RepositoriesFeature+Groups.swift` (`sidebarGroupsReducer`, chained `ifLet` for the `SidebarGroupNameFeature` name sheet, mirroring the worktree-customization pattern because the main `body` switch is at type-checker capacity). Entry points: "Move to Group" submenu (`SidebarMoveToGroupMenu`) on the git repo section ellipsis menu and the folder row context menu; rename/ungroup on the group header's context menu. Group actions' cache invalidations are declared in the `cacheInvalidations` map alongside the other sidebar layout actions.
...
- The fork is branded **Customcode**: `CFBundleDisplayName`/`CFBundleName` in `Info.plist`, `WindowTitle.appName`, the main `Window` scene title, and the app-menu "Customcode" / "Quit Customcode" entries in `supacodeApp.swift`. The app icon stays upstream's. `PRODUCT_NAME` (`supacode.app`), the bundle ID, deeplink scheme, and CLI name are deliberately unchanged so Sparkle updates, saved settings, and scripts keep working. Alert/settings prose still says "Supacode" on purpose — renaming it would bloat the upstream-merge surface. Keep all of this when merging upstream.
- `make release-personal` builds Release (`dist-personal`), signs the Sparkle feed with the private key at `~/.config/supacode-personal/sparkle_eddsa_private.key` (also in the login Keychain under account `supacode-personal`), notarizes via `xcrun notarytool`, staples, regenerates `appcast.xml` via Sparkle's `generate_appcast`, and uploads zip + appcast to the `personal` release with `gh`.
- Notarization auth prefers an App Store Connect API key (Team Key, Developer role): `~/.config/supacode-personal/AuthKey_<KEYID>.p8` plus the issuer UUID in `~/.config/supacode-personal/asc_issuer_id`. This is keychain-free, so it works in headless / locked-keychain sessions. When either file is missing it falls back to the `supacode-notary` keychain profile, which requires the login keychain to be unlocked (`security unlock-keychain ~/Library/Keychains/login.keychain-db`). Key ID and issuer live on disk, never in the public Makefile.
- Personal builds are Developer ID signed (`Developer ID Application: SiSprocom GmbH`, team `99R82368RE`) with hardened runtime + secure timestamps. `dist-personal` re-signs staged nested executables (bundled CLI, zmx, Sparkle helpers/XPC services) inside-out before signing the app, because notarization rejects any non-Developer-ID Mach-O and library validation requires team-consistent signatures. Never revert to plain ad-hoc for distributed builds: ad-hoc + hardened runtime crashes at launch on other machines (dyld rejects the embedded Sparkle), and ad-hoc without hardened runtime fails notarization.
